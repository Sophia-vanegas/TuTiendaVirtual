...
async def admin_add_producto(prod: ProductoSchema, db: Session = Depends(get_db)):
    new_prod = ProductoDB(**prod.model_dump(exclude={"created_at", "updated_at"}))
    db.add(new_prod)
    db.commit()
    db.refresh(new_prod)
    return new_prod
...
async def admin_update_producto(id: str, prod: ProductoSchema, db: Session = Depends(get_db)):
    ...
    for key, value in prod.model_dump(exclude={"id", "created_at", "updated_at"}).items():
...
async def update_profile(id: str, req: ProfileUpdate, db: Session = Depends(get_db)):
    ...
    if user.rol == "cliente":
        cliente = db.query(ClienteDB).filter(ClienteDB.id == id).first()
        if cliente:
            if req.nombre: cliente.nombre = req.nombre
            if req.email: cliente.email = req.email
            if req.cedula: cliente.cedula = req.cedula
            if req.apellidos: cliente.apellidos = req.apellidos
            if req.celular: cliente.celular = req.celular
            if req.direccion: cliente.direccion = req.direccion
